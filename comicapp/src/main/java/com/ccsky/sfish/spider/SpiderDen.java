...
import com.ccsky.io.UniFileOutputStreamPipe;
...
public final class SpiderDen {
    ...
    public static UniFile getGalleryDownloadDir(GalleryInfo galleryInfo) {
        ...
        if (dir != null) {
            // Read from DB
            String dirname = SkyDB.getDownloadDirname(galleryInfo.gid);
            if (null != dirname) {
                // Some dirname may be invalid in some version
                dirname = FileUtils.sanitizeFilename(dirname);
                SkyDB.putDownloadDirname(galleryInfo.gid, dirname);
            }
            ...
            // Create it
            if (null == dirname) {
                dirname = FileUtils.sanitizeFilename(galleryInfo.gid + "-" + SkyUtils.getSuitableTitle(galleryInfo));
                SkyDB.putDownloadDirname(galleryInfo.gid, dirname);
            }
    ...
    private boolean containInCache(int index) {
        ...
        String key = SkyCacheKeyFactory.getImageKey(mGid, index);
