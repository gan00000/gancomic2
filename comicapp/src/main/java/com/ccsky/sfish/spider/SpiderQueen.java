...
import com.ccsky.sfish.gallery.GalleryProvider2;
...
public final class SpiderQueen implements Runnable {
    ...
    private SpiderQueen(SkyApplication application, @NonNull GalleryInfo galleryInfo) {
        mHttpClient = SkyApplication.getOkHttpClient(application);
        mSpiderInfoCache = SkyApplication.getSpiderInfoCache(application);
    ...
    private SpiderInfo readSpiderInfoFromInternet() {
        try {
            ...
            Request request = new SkyRequestBuilder(SkyUrl.getGalleryDetailUrl(
                    mGalleryInfo.gid, mGalleryInfo.token, 0, false), SkyUrl.getReferer()).build();
    ...
    private String getPTokenFromInternet(int index) {
        ...
        try {
            String url = SkyUrl.getGalleryDetailUrl(
    ...
    private class SpiderWorker implements Runnable {
        ...
        private String getPageUrl(long gid, int index, String pToken,
            ...
            if (oldPageUrl != null) {
                ...
                pageUrl = SkyUrl.getPageUrl(gid, index, pToken);
            }
            // Add skipHathKey
            if (skipHathKey != null) {
                if (pageUrl.contains("?")) {
        ...
        private GalleryPageParser.Result fetchPageResultFromHtml(int index, String pageUrl) throws Throwable {
            GalleryPageParser.Result result = SkyEngine.getGalleryPage(null, mHttpClient, pageUrl, mGalleryInfo.gid, mGalleryInfo.token);
        ...
        private GalleryPageApiParser.Result fetchPageResultFromApi(long gid, int index, String pToken, String showKey, String previousPToken) throws Throwable {
            GalleryPageApiParser.Result result = SkyEngine.getGalleryPageApi(null, mHttpClient, gid, index, pToken, showKey, previousPToken);
            if (StringUtils.endsWith(result.imageUrl, URL_509_SUFFIX_ARRAY)) {
                // Get 509
                // Notify listeners
                notifyGet509(index);
        ...
        private boolean downloadImage(long gid, int index, String pToken, String previousPToken, boolean force) {
            ...
            for (int i = 0; i < 5; i++) {
                ...
                if (Settings.getDownloadOriginImage() && !TextUtils.isEmpty(originImageUrl)) {
                    ...
                    referer = SkyUrl.getPageUrl(gid, index, pToken);
                } else {
                ...
                try {
                    ...
                    Call call = mHttpClient.newCall(new SkyRequestBuilder(targetImageUrl, referer).build());
