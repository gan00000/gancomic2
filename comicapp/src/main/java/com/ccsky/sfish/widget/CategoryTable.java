...
package com.ccsky.sfish.widget;
...
import com.ccsky.widget.CheckTextView;
...
public class CategoryTable extends TableLayout implements View.OnLongClickListener {
    ...
    public void setCategory(int category) {
        mDoujinshi.setChecked(!NumberUtils.int2boolean(category & SkyConfig.DOUJINSHI), false);
        mManga.setChecked(!NumberUtils.int2boolean(category & SkyConfig.MANGA), false);
        mArtistCG.setChecked(!NumberUtils.int2boolean(category & SkyConfig.ARTIST_CG), false);
        mGameCG.setChecked(!NumberUtils.int2boolean(category & SkyConfig.GAME_CG), false);
        mWestern.setChecked(!NumberUtils.int2boolean(category & SkyConfig.WESTERN), false);
        mNonH.setChecked(!NumberUtils.int2boolean(category & SkyConfig.NON_H), false);
        mImageSets.setChecked(!NumberUtils.int2boolean(category & SkyConfig.IMAGE_SET), false);
        mCosplay.setChecked(!NumberUtils.int2boolean(category & SkyConfig.COSPLAY), false);
        mAsianPorn.setChecked(!NumberUtils.int2boolean(category & SkyConfig.ASIAN_PORN), false);
        mMisc.setChecked(!NumberUtils.int2boolean(category & SkyConfig.MISC), false);
    }
    ...
    public int getCategory() {
        int category = 0;
        if (!mDoujinshi.isChecked()) category |= SkyConfig.DOUJINSHI;
        if (!mManga.isChecked()) category |= SkyConfig.MANGA;
        if (!mArtistCG.isChecked()) category |= SkyConfig.ARTIST_CG;
        if (!mGameCG.isChecked()) category |= SkyConfig.GAME_CG;
        if (!mWestern.isChecked()) category |= SkyConfig.WESTERN;
        if (!mNonH.isChecked()) category |= SkyConfig.NON_H;
        if (!mImageSets.isChecked()) category |= SkyConfig.IMAGE_SET;
        if (!mCosplay.isChecked()) category |= SkyConfig.COSPLAY;
        if (!mAsianPorn.isChecked()) category |= SkyConfig.ASIAN_PORN;
        if (!mMisc.isChecked()) category |= SkyConfig.MISC;
        return category;
    }
