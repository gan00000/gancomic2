...
import com.ccsky.app.ListCheckBoxDialogBuilder;
...
import com.ccsky.sfish.download.SkyDownloadService;
...
//        }
    ...
    private static final class UpdateTask extends AsyncTask<Void, Void, JSONObject> {
        ...
        public UpdateTask(Activity activity, boolean feedback) {
            mActivity = activity;
            mHttpClient = SkyApplication.getOkHttpClient(activity);
            mFeedback = feedback;
        }
        ...
        protected JSONObject doInBackground(Void... params) {
            ...
            if (Settings.getBetaUpdateChannel()) {
                url = "http://www.sfish.com/update_beta.json";
            } else {
                url = "http://www.sfish.com/update.json";
            }
    ...
    private static void doAddToFavorites(Activity activity, GalleryInfo galleryInfo,
            int slot, SkyClient.Callback<Void> listener) {
        if (slot == -1) {
            SkyDB.putLocalFavorites(galleryInfo);
            listener.onSuccess(null);
        } else if (slot >= 0 && slot <= 9) {
            SkyClient client = SkyApplication.getEhClient(activity);
            EhRequest request = new EhRequest();
            request.setMethod(SkyClient.METHOD_ADD_FAVORITES);
    ...
    public static void addToFavorites(final Activity activity, final GalleryInfo galleryInfo,
            final SkyClient.Callback<Void> listener) {
    ...
    public static void removeFromFavorites(Activity activity, GalleryInfo galleryInfo,
            final SkyClient.Callback<Void> listener) {
        SkyDB.removeLocalFavorites(galleryInfo.gid);
        SkyClient client = SkyApplication.getEhClient(activity);
        EhRequest request = new EhRequest();
        request.setMethod(SkyClient.METHOD_ADD_FAVORITES);
    ...
    private static class DelegateFavoriteCallback implements SkyClient.Callback<Void> {

        private final SkyClient.Callback<Void> delegate;
        private final GalleryInfo info;
        private final String newFavoriteName;
        private final int slot;

        DelegateFavoriteCallback(SkyClient.Callback<Void> delegate, GalleryInfo info,
        ...
        public void onSuccess(Void result) {
            ...
            SkyApplication.getFavouriteStatusRouter().modifyFavourites(info.gid, slot);
        }
    ...
    public static void startDownload(final SkyMainActivity activity, final GalleryInfo galleryInfo, boolean forceDefault) {
    ...
    public static void startDownload(final SkyMainActivity activity, final List<GalleryInfo> galleryInfos, boolean forceDefault) {
        final DownloadManager dm = SkyApplication.getDownloadManager(activity);
        ...
        if (!toStart.isEmpty()) {
            Intent intent = new Intent(activity, SkyDownloadService.class);
            intent.setAction(SkyDownloadService.ACTION_START_RANGE);
            intent.putExtra(SkyDownloadService.KEY_GID_LIST, toStart);
            activity.startService(intent);
        }

        if (toAdd.isEmpty()) {
            activity.showTip(R.string.added_to_download_list, SkyBaseScene.LENGTH_SHORT);
            return;
        }
        ...
        if (justStart) {
            ...
            for (GalleryInfo gi : toAdd) {
                Intent intent = new Intent(activity, SkyDownloadService.class);
                intent.setAction(SkyDownloadService.ACTION_START);
                intent.putExtra(SkyDownloadService.KEY_LABEL, label);
                intent.putExtra(SkyDownloadService.KEY_GALLERY_INFO, gi);
                activity.startService(intent);
            }
            // Notify
            activity.showTip(R.string.added_to_download_list, SkyBaseScene.LENGTH_SHORT);
