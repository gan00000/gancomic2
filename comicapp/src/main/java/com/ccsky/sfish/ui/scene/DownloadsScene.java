...
public class DownloadsScene extends ToolbarScene
    ...
    public void onDestroy() {
        ...
        if (null == manager) {
            Context context = getContext2();
            if (null != context) {
                manager = SkyApplication.getDownloadManager(context);
            }
        } else {
...
//                intent.setAction(SkyDownloadService.ACTION_START_ALL);
...
//                }
        ...
        List<DownloadLabel> list = SkyApplication.getDownloadManager(context).getLabelList();
    ...
    public boolean onItemClick(EasyRecyclerView parent, View view, int position, long id) {
        ...
        if (recyclerView.isInCustomChoice()) {
            ...
            intent.putExtra(SkyGalleryActivity.KEY_GALLERY_INFO, list.get(position));
        ...
        if (0 == position) {
            ...
            switch (position) {
                case 1: { // Start
                    ...
                    intent.putExtra(SkyDownloadService.KEY_GID_LIST, gidList);
                    activity.startService(intent);
                    // Cancel check mode
                ...
                case 4: {// Move
                    List<DownloadLabel> labelRawList = SkyApplication.getDownloadManager(context).getLabelList();
    ...
    private class DeleteDialogHelper implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (checked) {
                ...
                SkyDB.removeDownloadDirname(mGalleryInfo.gid);
                // Delete file
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            if (checked) {
                ...
                for (DownloadInfo info: mDownloadInfoList) {
                    // Remove download path
                    SkyDB.removeDownloadDirname(info.gid);
                    // Put file
    ...
    private class MoveDialogHelper implements DialogInterface.OnClickListener {
        ...
        public void onClick(DialogInterface dialog, int which) {
            ...
            SkyApplication.getDownloadManager(context).changeLabel(mDownloadInfoList, label);
        }
    }
    ...
    private class DownloadHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        ...
        public void onClick(View v) {
            ...
            if (thumb == v) {
                ...
                intent.putExtra(SkyDownloadService.KEY_GALLERY_INFO, list.get(index));
                activity.startService(intent);
            } else if (stop == v) {
                if (null != mDownloadManager) {
    ...
    private class DownloadAdapter extends RecyclerView.Adapter<DownloadHolder> {
        ...
        public void onBindViewHolder(DownloadHolder holder, int position) {
            ...
            holder.thumb.load(SkyCacheKeyFactory.getThumbKey(info.gid), info.thumb,
                    new ThumbDataContainer(info), true);
            holder.title.setText(SkyUtils.getSuitableTitle(info));
            holder.uploader.setText(info.uploader);
            holder.rating.setRating(info.rating);
            TextView category = holder.category;
            String newCategoryText = SkyUtils.getCategory(info.category);
            if (!newCategoryText.equals(category.getText())) {
                category.setText(newCategoryText);
                category.setBackgroundColor(SkyUtils.getCategoryColor(info.category));
            }
