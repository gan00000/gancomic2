...
import com.ccsky.sfish.client.exception.OffensiveException;
import com.ccsky.sfish.client.exception.ParseException;
import com.ccsky.sfish.client.exception.PiningException;
...
import com.ccsky.sfish.client.data.LargePreviewSet;
import com.ccsky.sfish.client.data.NormalPreviewSet;
...
public class GalleryDetailParser {
    ...
    public static GalleryDetail parse(String body) throws SkyException {
        ...
        if (m.find()) {
            throw new SkyException(m.group(1));
        }
    ...
    private static void parseDetail(GalleryDetail gd, Document d, String body) throws ParseException {
        ...
        try {
            ...
            try {
                ...
                gd.category = SkyUtils.getCategory(ce.text());
            } catch (Throwable e) {
                ExceptionUtils.throwIfFatal(e);
                gd.category = SkyUtils.UNKNOWN;
            }
    ...
    private static String parseCoverStyle(String str) {
        ...
        if (matcher.find()) {
            return SkyUtils.handleThumbUrlResolution(matcher.group(3));
        } else {
            return "";
        }
    }
     ...
     */
    @NonNull
    private static GalleryTagGroup[] parseTagGroups(String body) throws SkyException {
        ...
        try {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (Settings.getFixThumbUrl()) {
                    imageUrl = SkyUrl.getFixedPreviewThumbUrl(imageUrl);
                }
