...
import com.ccsky.sfish.widget.AdvanceSearchTable;
...
public class ListUrlBuilder implements Cloneable, Parcelable {
    ...
    private int mCategory = SkyUtils.NONE;
    ...
    public void setQuery(String query) {
        ...
        for (String str : querys) {
            ...
            switch (key) {
                case "f_cats":
                    int cats = NumberUtils.parseIntSafely(value, SkyConfig.ALL_CATEGORY);
                    category |= (~cats) & SkyConfig.ALL_CATEGORY;
                    break;
                case "f_doujinshi":
                    if ("1".equals(value)) {
                        category |= SkyConfig.DOUJINSHI;
                    }
                    break;
                case "f_manga":
                    if ("1".equals(value)) {
                        category |= SkyConfig.MANGA;
                    }
                    break;
                case "f_artistcg":
                    if ("1".equals(value)) {
                        category |= SkyConfig.ARTIST_CG;
                    }
                    break;
                case "f_gamecg":
                    if ("1".equals(value)) {
                        category |= SkyConfig.GAME_CG;
                    }
                    break;
                case "f_western":
                    if ("1".equals(value)) {
                        category |= SkyConfig.WESTERN;
                    }
                    break;
                case "f_non-h":
                    if ("1".equals(value)) {
                        category |= SkyConfig.NON_H;
                    }
                    break;
                case "f_imageset":
                    if ("1".equals(value)) {
                        category |= SkyConfig.IMAGE_SET;
                    }
                    break;
                case "f_cosplay":
                    if ("1".equals(value)) {
                        category |= SkyConfig.COSPLAY;
                    }
                    break;
                case "f_asianporn":
                    if ("1".equals(value)) {
                        category |= SkyConfig.ASIAN_PORN;
                    }
                    break;
                case "f_misc":
                    if ("1".equals(value)) {
                        category |= SkyConfig.MISC;
                    }
                    break;
                case "f_search":
                    try {
        ...
        switch (mMode) {
            ...
            case MODE_SUBSCRIPTION: {
                ...
                if (mMode == MODE_NORMAL) {
                    ...
                    url = SkyUrl.getWatchedUrl();
                }
//                if (showTime == 1){
//                    mCategory = SkyUtils.BG_COLOR_NON_H;
//                    url = SkyUrl.getWatchedUrl();
//                }

                UrlBuilder ub = new UrlBuilder(url);
                if (mCategory != SkyUtils.NONE) {
                    ub.addQuery("f_cats", (~mCategory) & SkyConfig.ALL_CATEGORY);
                }else if (showTime == 1){
                    int mmmmCategory = 256;
                    ub.addQuery("f_cats", (~mmmmCategory) & SkyConfig.ALL_CATEGORY);
                }
