...
import com.ccsky.network.CookieRepository;
...
public class EhCookieStore extends CookieRepository {
    ...
    public static final Cookie sTipsCookie =
            new Cookie.Builder()
                    .name(SkyConfig.KEY_CONTENT_WARNING)
                    .value(SkyConfig.CONTENT_WARNING_NOT_SHOW)
                    .domain(SkyUrl.DOMAIN_E)
                    .path("/")
    ...
    public boolean hasSignedIn() {
        HttpUrl url = HttpUrl.parse(SkyUrl.HOST_E);
    ...
    public List<Cookie> loadForRequest(HttpUrl url) {
        ...
        boolean checkTips = domainMatch(url, SkyUrl.DOMAIN_E);
        ...
        if (checkTips) {
            ...
            for (Cookie cookie: cookies) {
                ...
                if (SkyConfig.KEY_CONTENT_WARNING.equals(name)) {
                    continue;
                }
                if (SkyConfig.KEY_UCONFIG.equals(name)) {
                    continue;
                }
                result.add(cookie);
            }
