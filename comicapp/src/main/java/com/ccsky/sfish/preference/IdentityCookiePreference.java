...
import com.ccsky.preference.MessagePreference;
...
public class IdentityCookiePreference extends MessagePreference {
    ...
    private void init() {
        EhCookieStore store = SkyApplication.getEhCookieStore(getContext());
        List<Cookie> eCookies = store.getCookies(HttpUrl.get(SkyUrl.HOST_E));
        List<Cookie> exCookies = store.getCookies(HttpUrl.get(SkyUrl.HOST_EX));
