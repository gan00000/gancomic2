...
import com.ccsky.sfish.SkyProxySelector;
import com.ccsky.sfish.Settings;
import com.ccsky.network.InetValidator;
import com.ccsky.preference.DialogPreference;
...
public class ProxyPreference extends DialogPreference implements View.OnClickListener {
    ...
    private void updateSummary(int type, String ip, int port) {
        if ((type == SkyProxySelector.TYPE_HTTP || type == SkyProxySelector.TYPE_SOCKS)
                && (TextUtils.isEmpty(ip) || !InetValidator.isValidInetPort(port)) ) {
            type = SkyProxySelector.TYPE_SYSTEM;
        }
    ...
    public void onClick(View v) {
        ...
        SkyApplication.getEhProxySelector(getContext()).updateProxy();
